...
const schema: FieldConfig<PayloadDateBlock>['schema'] = (m) => {
  switch (m.mode) {
    case 'single':
      return m.required ? z.date({ error: REQUIRED_MESSAGE }) : z.date().optional();
    case 'multiple':
      return m.required
        ? z.date().array().min(1, { error: REQUIRED_MESSAGE })
        : z.date().array().optional();
    case 'range':
      return z.object({
        from: m.required ? z.date({ error: REQUIRED_MESSAGE }) : z.date().optional(),
        to: z.date().optional(),
      });
  }
};
